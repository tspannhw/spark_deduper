...
DEFAULT_SETTINGS_FILE = 'conf/settings.yaml'
...
def parse_arguments():
    ...
    parser.add_argument('-s', '--settings_file', type=str, help="Path of the yaml file with settings", required=False)
    ARGS = parser.parse_args()

    return ARGS

def get_settings(ARGS):
    # Settings provided with the -s argument
    if ARGS.settings_file is not None:
        if os.path.isfile(ARGS.settings_file):
            with open(ARGS.settings_file, 'r') as f:
                settings = yaml.load(f)
        else:
            raise BaseException("The settings file provided as argument does not exist (%s)" % ARGS.settings_file)
    # Default settings
    else:
        if os.path.isfile(DEFAULT_SETTINGS_FILE):
            ...
            raise BaseException("The default settings file does not exist (%s)" % DEFAULT_SETTINGS_FILE)
